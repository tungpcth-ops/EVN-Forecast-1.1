...
def build_docx(series, forecasts, backtest, top100=None, state=None, outage_monthly=None, outage_details=None, title="BÁO CÁO EVN FORECAST 1.2"):
    bio = io.BytesIO()
    doc = Document()
    ...
    doc.add_heading("3. Dự báo", level=1)
    ...
    doc.add_heading("4. Kiểm định mô hình", level=1)
    ...
    if top100 is not None and not top100.empty:
        doc.add_heading("5. Top 10 khách hàng ảnh hưởng", level=1)
        use = top100.head(10)
    ...
    doc.add_paragraph("\nBáo cáo được tạo tự động bởi EVN Forecast 1.2 Web.")
    doc.save(bio)
    return bio.getvalue()


def build_pdf(series, forecasts, backtest, top100=None, state=None, outage_monthly=None, outage_details=None, title="EVN Forecast 1.2"):
    bio = io.BytesIO()
    ...
    if top100 is not None and not top100.empty:
        ...
        story.append(tbl)
    story += [Spacer(1,10), Paragraph("Bao cao duoc tao tu dong boi EVN Forecast 1.2 Web.", styles["BodyText"])]
    doc.build(story)
    return bio.getvalue()
